...
declare @table_name varchar(255) = 'MsrDetails_MonthEnd'
...
case
    ...
    case
        when type_name(columns.system_type_id) like '%char%' or type_name(columns.system_type_id) like '%binary%' then type_name(columns.system_type_id) + '(' + case when columns.max_length = -1 then 'max' else cast(columns.max_length as varchar(255)) end + ')'
...
when indexes.is_primary_key = 0 and indexes.type_desc = 'CLUSTERED' then 'alter table ' + @table_full_name + ' add constraint ' + indexes.name + IIF(indexes.is_unique = 1, ' unique ', '') + ' clustered index '
else 'create index ' + iif(indexes.is_unique = 1, ' unique ', '') + quotename(indexes.name) + ' on ' + @table_full_name end,
...
column_names,
data_space_name,
create_statement = create_statement_prefix + '(' + column_names + ')' + data_space_append,
drop_statement,
rename_statement
from sys.indexes
...
parent_table_schema = object_schema_name(foreign_keys.parent_object_id),
parent_table_name = object_name(foreign_keys.parent_object_id),
parent_column_name = parent_column.name,
referenced_table_schema = object_schema_name(foreign_keys.referenced_object_id),
referenced_table_name = object_name(foreign_keys.referenced_object_id),
referenced_column_name = referenced_column.name,
create_statement = 'alter table ' + quotename(object_schema_name(foreign_keys.parent_object_id)) + '.' + quotename(object_name(foreign_keys.parent_object_id)) + ' add constraint ' + foreign_keys.name + ' foreign key (' + parent_column.name + ') references ' + quotename(object_schema_name(foreign_keys.referenced_object_id)) + '.' + object_name(foreign_keys.referenced_object_id) + ' (' + quotename(referenced_column.name) + ')',
drop_statement = 'alter table ' + quotename(object_schema_name(foreign_keys.parent_object_id)) + '.' + quotename(object_name(foreign_keys.parent_object_id)) + ' drop constraint ' + quotename(foreign_keys.name),
rename_statement = 'exec sp_rename ' + quotename(quotename(object_schema_name(foreign_keys.object_id), '''') + '.' + quotename(foreign_keys.name), '''') + ', ''FK_' + object_name(foreign_keys.parent_object_id) + '_' + object_name(foreign_keys.referenced_object_id) + ''''
